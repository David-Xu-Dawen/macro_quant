"""读取 model prediction 目录下的回测结果与图表，供 Web 展示。"""
...
MP_DIR = ROOT / "model prediction"
OUTPUT_DIR = MP_DIR / "output"
FIGURES_DIR = OUTPUT_DIR / "figures"

# 复用 model prediction/config 的档位定义
...
def _pct(x: float | None, digits: int = 1) -> str | None:
    if x is None or (isinstance(x, float) and not pd.notna(x)):
        return None
    return f"{100 * float(x):.{digits}f}%"


def _num(x: float | None, digits: int = 2) -> float | None:
    if x is None or (isinstance(x, float) and not pd.notna(x)):
        return None
...
def summarize(aggression: str | None = None) -> dict[str, Any]:
    ...
    if not out_dir.exists():
        raise FileNotFoundError(
            f"档位「{profile['label']}」结果目录不存在: {out_dir}。"
            f"请先运行 python run_aggression_profiles.py"
        )
    metrics_path = out_dir / "rolling_metrics.json"
    if not metrics_path.exists():
        raise FileNotFoundError(
            f"档位「{profile['label']}」缺少 rolling_metrics.json。"
            f"请先运行: cd 'model prediction' && python3 run_aggression_profiles.py --only {key}"
        )
    ...
    if not feat_imp.empty and {"feature", "importance"}.issubset(feat_imp.columns):
        for _, row in feat_imp.sort_values("importance", ascending=False).head(12).iterrows():
            top_features.append(
                {
    ...
    return {
        ...
        "universe": [{"id": u, "name": ASSET_CN.get(u, u)} for u in universe],
        "metrics": {
